...
public class Umap {
   ...
   *                          Distances to nearest neighbors for each samples.
   *                          Each row should be a
   *                          sorted list of distances to a given samples nearest
   *                          neighbors.
   * @param k                 The number of nearest neighbors to approximate for.
   * @param nIter             We need to binary search for the correct distance
   *                          value. This is the
   *                          max number of iterations to use in such a search.
   * @param localConnectivity The local connectivity required; i.e., the number of
   *                          nearest
   *                          neighbors that should be assumed to be connected at
   *                          a local level.
   *                          The higher this value the more connected the
   *                          manifold becomes
   *                          locally. In practice this should be not more than
   *                          the local intrinsic
   *                          dimension of the manifold.
   * @param bandwidth         The target bandwidth of the kernel, larger values
   *                          will produce
   ...
   */
  private static float[][] smoothKnnDist(final float[][] distances, final float k, final int nIter,
      final int localConnectivity, final float bandwidth) {
  ...
  /**
   * Compute the <code>nNeighbors</code> nearest points for each data point in
   * <code>instances</code>
   * under <code>metric</code>. This may be exact, but more likely is approximated
   * via
   * nearest neighbor descent.
   * 
   * @param instances  The input data to compute the k-neighbor graph of.
   * @param nNeighbors The number of nearest neighbors to compute for each sample
   *                   in <code>instances</code>.
   ...
   *         The indices on the <code>nNeighbors</code> closest points in the
   *         dataset.
   *         knnDists: array of shape <code>(nSamples, nNeighbors)</code>
   *         The distances to the <code>nNeighbors</code> closest points in the
   *         dataset.
   */
  static IndexedDistances nearestNeighbors(final Matrix instances, final int nNeighbors, final Metric metric,
      boolean angular, final Random random, final int threads, final boolean verbose) {
    if (verbose) {
    ...
    if (metric.equals(PrecomputedMetric.SINGLETON)) {
      ...
      if (instances instanceof CsrMatrix) {
        ...
        //// }
        // } else {
        // throw new IllegalArgumentException("Metric " + metric + " not supported for
        // sparse data");
        // }
        throw new UnsupportedOperationException();
        // metric_nn_descent = Sparse.make_sparse_nn_descent(distanceFunc,
        // tuple(metricKwds.values()));
        //
        ...
        // }
        // final Object[] nn = NearestNeighborDescent.metric_nn_descent(Y.indices,
        // Y.indptr, Y.data, Y.rows(), nNeighbors, random, /*max_candidates=*/60,
        // /*rp_tree_init=*/true, /*leaf_array=*/leaf_array, /*n_iters=*/n_iters,
        // verbose);
        // knnIndices = (int[][]) nn[0];
        // knnDists = (float[][]) nn[1];
      } else {
        final NearestNeighborDescent metricNearestNeighborsDescent = threads == 1 ? new NearestNeighborDescent(metric)
        ...
        final Heap nn = metricNearestNeighborsDescent.descent(instances, nNeighbors, random, 60, true, nIters,
            rpForest);
      ...
      if (MathUtils.containsNegative(knnIndices)) {
        Utils.message(
            "Failed to correctly find nearest neighbors for some samples. Results may be less than ideal. Try re-running with different parameters.");
      }
    }
    if (verbose) {
   ...
   *                   The indices on the <code>nNeighbors</code> closest points
   *                   in the dataset.
   * @param knnDists   array of shape <code>(nSamples, nNeighbors)</code>
   *                   The distances to the <code>nNeighbors</code> closest points
   *                   in the dataset.
   * @param sigmas     array of shape <code>(nSamples)</code>
   *                   The normalization factor derived from the metric tensor
   *                   approximation.
   ...
   */
  static CooMatrix computeMembershipStrengths(final int[][] knnIndices, final float[][] knnDists, final float[] sigmas,
      final float[] rhos, final int rowCount, final int colCount) {
   ...
   * @param nNeighbors        The number of neighbors to use to approximate
   *                          geodesic distance.
   *                          Larger numbers induce more global estimates of the
   *                          manifold that can
   *                          miss finer detail, while smaller values will focus
   *                          on fine manifold
   *                          structure to the detriment of the larger picture.
   * @param random            Randomness source
   * @param metric            The metric to use to compute distances in high
   *                          dimensional space.
   * @param knnIndices        array of shape <code>(nSamples, nNeighbors)</code>
   *                          or null.
   *                          If the k-nearest neighbors of each point has already
   *                          been calculated
   *                          you can pass them in here to save computation time.
   *                          This should be
   *                          an array with the indices of the k-nearest neighbors
   *                          as a row for
   *                          each data point.
   * @param knnDists          array of shape <code>(nSamples, nNeighbors)</code>
   *                          or null.
   ...
   *                          you can pass them in here to save computation time.
   *                          This should be
   *                          an array with the distances of the k-nearest
   *                          neighbors as a row for
   *                          each data point.
   * @param angular           Whether to use angular/cosine distance for the
   *                          random projection
   *                          forest for seeding NN-descent to determine
   *                          approximate nearest
   *                          neighbors.
   * @param setOpMixRatio     Interpolate between (fuzzy) union and intersection
   *                          as the set operation
   *                          used to combine local fuzzy simplicial sets to
   *                          obtain a global fuzzy
   *                          simplicial sets. Both fuzzy set operations use the
   *                          product t-norm.
   *                          The value of this parameter should be between 0.0
   *                          and 1.0; a value of
   *                          1.0 will use a pure fuzzy union, while 0.0 will use
   *                          a pure fuzzy
   *                          intersection.
   * @param localConnectivity The local connectivity required; i.e., the number of
   *                          nearest
   ...
   * @param verbose           Whether to report information on the current
   *                          progress of the algorithm.
   * @return A fuzzy simplicial set represented as a sparse matrix. The
   *         <code>(i, j)</code>
   *         entry of the matrix represents the membership strength of the
   *         1-simplex between the ith and jth sample points.
   */
  static Matrix fuzzySimplicialSet(final Matrix instances, final int nNeighbors, final Random random,
      final Metric metric, int[][] knnIndices, float[][] knnDists, final boolean angular, final float setOpMixRatio,
      final int localConnectivity, final int threads, final boolean verbose) {
    ...
    final Matrix result = computeMembershipStrengths(knnIndices, knnDists, sigmas, rhos, instances.rows(),
        instances.rows()).eliminateZeros();
    final Matrix prodMatrix = result.hadamardMultiplyTranspose();

    return result.addTranspose().subtract(prodMatrix).multiply(setOpMixRatio)
        .add(prodMatrix.multiply(1.0F - setOpMixRatio)).eliminateZeros();
  }
   ...
   * 
   * @param simplicialSet The simplicial set for which to recalculate with respect
   ...
   * @param unknownDist   The distance an unknown label (-1) is assumed to be from
   *                      any point.
   * @param farDist       The distance between unmatched labels.
   * @return The resulting intersected fuzzy simplicial set.
   */
  private static Matrix categoricalSimplicialSetIntersection(final CooMatrix simplicialSet, final float[] target,
      final float unknownDist, final float farDist) {
    simplicialSet.fastIntersection(target, unknownDist, farDist);
    return resetLocalConnectivity(simplicialSet.eliminateZeros());
  }

  private static Matrix generalSimplicialSetIntersection(final Matrix simplicialSet1, final Matrix simplicialSet2,
      final float weight) {
   ...
   * @param tailEmbedding      array of shape
   *                           <code>(sourceSamples, nComponents)</code>
   *                           The reference embedding of embedded points. If not
   *                           embedding new
   *                           previously unseen points with respect to an
   *                           existing embedding this
   *                           is simply the <code>headEmbedding</code> (again);
   *                           otherwise it provides the
   *                           existing embedding to embed with respect to.
   * @param head               array of shape (n_1_simplices)
   *                           The indices of the heads of 1-simplices with
   *                           non-zero membership.
   * @param tail               array of shape (n_1_simplices)
   *                           The indices of the tails of 1-simplices with
   *                           non-zero membership.
   * @param nEpochs            The number of training epochs to use in
   *                           optimization.
   * @param nVertices          The number of vertices (0-simplices) in the
   *                           dataset.
   * @param epochsPerSample    A float value of the number of epochs per
   *                           1-simplex. 1-simplices with
   *                           weaker membership strength will have more epochs
   *                           between being sampled.
   * @param a                  Parameter of differentiable approximation of right
   *                           adjoint functor
   * @param b                  Parameter of differentiable approximation of right
   *                           adjoint functor
   * @param random             Random source
   * @param gamma              Weight to apply to negative samples.
   * @param initialAlpha       Initial learning rate for the SGD.
   * @param negativeSampleRate Number of negative samples to use per positive
   *                           sample.
   * @param verbose            Whether to report information on the current
   *                           progress of the algorithm.
   * @return array of shape <code>(nSamples, nComponents)</code> The optimized
   *         embedding.
   */
  private Matrix optimizeLayout(final Matrix headEmbedding, final Matrix tailEmbedding, final int[] head,
      final int[] tail, final int nEpochs, final int nVertices, final float[] epochsPerSample, final float a,
      final float b, final Random random, final float gamma, final float initialAlpha, final float negativeSampleRate,
    ...
    for (int n = 0; n < nEpochs; ++n) {
      for (int i = 0; i < epochsPerSample.length; ++i) {
        if (epochOfNextSample[i] <= n) {
          ...
          float gradCoeff;
          if (distSquared > 0.0) {
            gradCoeff = (float) ((-2.0 * a * b * Math.pow(distSquared, b - 1.0))
                / (a * Math.pow(distSquared, b) + 1.0));
          } else {
            gradCoeff = 0;
          }
   ...
   * @param graphIn            The 1-skeleton of the high dimensional fuzzy
   *                           simplicial set as
   *                           represented by a graph for which we require a
   *                           sparse matrix for the
   *                           (weighted) adjacency matrix.
   * @param nComponents        The dimensionality of the euclidean space into
   ...
   *                           adjoint functor.
   * @param gamma              Weight to apply to negative samples.
   * @param negativeSampleRate The number of negative samples to select per
   *                           positive sample
   *                           in the optimization process. Increasing this value
   *                           will result
   *                           in greater repulsive force being applied, greater
   *                           optimization
   *                           cost, but slightly more accuracy.
   * @param nEpochs            The number of training epochs to be used in
   *                           optimizing the
   *                           low dimensional embedding. Larger values result in
   *                           more accurate
   *                           embeddings. If 0 is specified a value will be
   *                           selected based on
   *                           the size of the input dataset (200 for large
   *                           datasets, 500 for small).
   * @param init               How to initialize the low dimensional embedding.
   *                           Options are:
   *                           * 'spectral': use a spectral embedding of the fuzzy
   *                           1-skeleton
   *                           * 'random': assign initial embedding positions at
   *                           random.
   *                           * A numpy array of initial embedding positions.
   * @param random             random source
   * @param metric             The metric used to measure distance in high
   *                           dimensional space; used if
   *                           multiple connected components need to be layed out.
   * @param verbose            Whether to report information on the current
   *                           progress of the algorithm.
   * @return array of shape <code>(nSamples, nComponents)</code>
   *         The optimized of <code>graph</code> into an <code>nComponents</code>
   *         dimensional
   *         Euclidean space.
   */
  private Matrix simplicialSetEmbedding(Matrix data, Matrix graphIn, int nComponents, float initialAlpha, float a,
      float b, float gamma, int negativeSampleRate, int nEpochs, float[][] init, Random random, Metric metric,
      boolean verbose) {
    ...
    if (init == null) {
      embedding = new DefaultMatrix(MathUtils.uniform(random, -10, 10,
          graph.rows(), nComponents));
    } else {
      embedding = new DefaultMatrix(init);
    }

    // if ("random".equals(init)) {
    // embedding = random.uniform(low = -10.0, high = 10.0, size = (graph.rows(),
    // nComponents)).astype(np.float32);
    // embedding = new DefaultMatrix(MathUtils.uniform(random, -10, 10,
    // graph.rows(), nComponents));
    // } else if ("spectral".equals(init)) {
    // throw new UnsupportedOperationException();
    // // We add a little noise to avoid local minima for optimization to come
    // float[][] initialisation = Spectral.spectral_layout(data, graph, nComponents,
    // random, /*metric=*/metric, /*metric_kwds=*/metric_kwds);
    // float expansion = 10.0 / Math.abs(initialisation).max();
    // embedding = (MathUtils.multiply(initialisation,
    // expansion)).astype(np.float32) + random.normal(scale = 0.0001, size
    // =[graph.rows(), nComponents]).astype(np.float32);
    // } else {
    ...
    // embedding = init_data + random.normal(scale = 0.001 * nndist, size =
    // init_data.shape).astype(np.float32);
    // } else {
    // embedding = init_data;
    // }
    // }
    ...
    return optimizeLayout(embedding, embedding, head, tail, nEpochs, nVertices, epochsPerSample, a, b, random, gamma,
        initialAlpha, negativeSampleRate, verbose);
  }
   ...
   * relative to the <code>spread</code> value, which determines the scale at
   * which
  ...
  /**
   * Set the effective scale of embedded points. In combination with
   * <code>minDist</code>
  ...
  /**
   * Set the metric used to measure distance for a target array is using
   * supervised
   * dimension reduction. By default this is
   * <code>CategoricalMetric.SINGLETON</code> which will measure
   * distance in terms of whether categories match or are different. Furthermore,
   * if semi-supervised is required target values of -1 will be treated as
   * unlabelled under the <code>CategoricalMetric</code> metric. If the target
   * array takes
  ...
  /**
   * Set the target metric by name (see <code>setMetric</code> for a list of
   * values).
   * 
   * @param targetMetric target metric
   */
  ...
  private void validateParameters() {
    ...
    // throw new IllegalArgumentException("init ndarray must match nComponents
    // value");
    // }
  }
   ...
   * 
   * @param instances array of shape <code>(nSamples, nFeatures)</code> or
   *                  <code>(nSamples, nSamples)</code>
   *                  If the metric is 'precomputed' instances must be a square
   *                  distance
   *                  matrix. Otherwise it contains a sample per row. If the
   *                  method
   *                  is 'exact',
   * @param y         array of shape <code>(nSamples)</code>
   *                  A target array for supervised dimension reduction. How this
   *                  is
   *                  handled is determined by parameters UMAP was instantiated
   *                  with.
   *                  The relevant metric is <code>mTargetMetric</code>.
   * @throws IllegalArgumentException if the matrix contains non-finite elements.
   */
  private void fit(Matrix instances, float[] y, float[][] init) {
    ...
    if (instances.rows() < SMALL_PROBLEM_THRESHOLD) {
      ...
      mGraph = fuzzySimplicialSet(dmat, mRunNNeighbors, mRandom, PrecomputedMetric.SINGLETON, null, null,
          mAngularRpForest, mSetOpMixRatio, mLocalConnectivity, mThreads, mVerbose);
    } else {
      mSmallData = false;
      // Standard case
      final IndexedDistances nn = nearestNeighbors(instances, mRunNNeighbors, mMetric, mAngularRpForest, mRandom,
          mThreads, mVerbose);
      mKnnIndices = nn.getIndices();
      mKnnDists = nn.getDistances();
      mRpForest = nn.getForest();

      mGraph = fuzzySimplicialSet(instances, mNNeighbors, mRandom, mMetric, mKnnIndices, mKnnDists, mAngularRpForest,
          mSetOpMixRatio, mLocalConnectivity, mThreads, mVerbose);
    ...
    if (y != null) {
      if (instances.length() != y.length) {
        throw new IllegalArgumentException(
            "Length of x =  " + instances.length() + ", length of y = " + y.length + ", while it must be equal.");
      }
      if (CategoricalMetric.SINGLETON.equals(mTargetMetric)) {
        ...
        if (y.length < SMALL_PROBLEM_THRESHOLD) {
          ...
          targetGraph = fuzzySimplicialSet(ydmat, targetNNeighbors, mRandom, PrecomputedMetric.SINGLETON, null, null,
              false, 1, 1, mThreads, false);
        } else {
          // Standard case
          targetGraph = fuzzySimplicialSet(MathUtils.promoteTranspose(y), targetNNeighbors, mRandom, mTargetMetric,
              null, null, false, 1, 1, mThreads, false);
        }
    ...
    mEmbedding = simplicialSetEmbedding(mRawData, mGraph, mNComponents, mInitialAlpha, mRunA, mRunB, mRepulsionStrength,
        mNegativeSampleRate, nEpochs, init, mRandom, mMetric, mVerbose);
   ...
   * @param instances array of shape <code>(nSamples, nFeatures)</code> or
   *                  <code>(nSamples, nSamples)</code>
   *                  If the metric is <code>PrecomputedMetric.SINGLETON</code>
   ...
   */
  public Matrix fitTransform(final Matrix instances, final float[] y, float[][] init) {
    fit(instances, y, init);
    return mEmbedding;
  }
   ...
   *         Embedding of the training data in low-dimensional space.
   */
  public Matrix fitTransform(final Matrix instances, float[][] init) {
   ...
   *         Embedding of the training data in low-dimensional space.
   */
  public float[][] fitTransform(final float[][] instances, float init) {
   ...
   *         Embedding of the training data in low-dimensional space.
   */
  public double[][] fitTransform(final double[][] instances, float[][] init) {
    ...
    final Matrix result = fitTransform(new DefaultMatrix(input), null, init);
  ...
  public Matrix transform(Matrix instances) {
    ...
    if (mSmallData) {
      ...
      final Heap init = NearestNeighborDescent.initialiseSearch(mRpForest, mRawData, instances,
          (int) (mRunNNeighbors * mTransformQueueSize), mSearch, mRandom);
    ...
    final Matrix matrix = optimizeLayout(embedding, mEmbedding.copy(), head, tail, nEpochs, graph.cols(),
        epochsPerSample, mRunA, mRunB, mRandom, mRepulsionStrength, mInitialAlpha, mNegativeSampleRate, mVerbose);
